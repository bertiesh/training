...
import com.example.training.strategy.UploadStrategy;
import com.example.training.util.FileUtils;
...
public abstract class AbstractUploadStrategyImpl implements UploadStrategy {
    ...
    public List<String> uploadFiles(MultipartFile[] files, String path) {
        List<String> fileAccessUrls = new ArrayList<>();
        for (MultipartFile file : files) {
            try {
                ...
                // 返回文件访问路径
                fileAccessUrls.add(getFileAccessUrl(path + fileName));
            } catch (Exception e) {
                e.printStackTrace();
                throw new BizException("文件上传失败");
            }
        }
        return fileAccessUrls;
    }
